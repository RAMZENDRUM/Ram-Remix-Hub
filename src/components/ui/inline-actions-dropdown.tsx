...
interface InlineActionsDropdownProps {
    /** optional className to position the wrapper */
    className?: string;
}

export const InlineActionsDropdown = ({ className }: InlineActionsDropdownProps) => {
    const [favorite, setFavorite] = useState(false);
    const [animating, setAnimating] = useState(false);

    useEffect(() => {
        if (animating) {
            const timeout = setTimeout(() => setAnimating(false), 100);
    ...
    const handleStart = () => setHolding(true);
    const handleEnd = () => {
        setHolding(false);
        setElapsedTime(0);
    };
    ...
    return (
        <div
            className={clsx(
                "flex w-52 flex-col items-start gap-y-1 rounded-lg border border-neutral-300 bg-neutral-200/10 p-1 dark:border-neutral-700 dark:bg-neutral-800",
                className
            )}
        >
            {/* Favorite toggle row */}
            ...
            {/* Destructive action with hold-to-confirm */}
            <button
